...
public class AmazonGiftingGroups {
    public static void main(String[] args) {
        ...
        System.out.println( findSubscriberGroups(arrayList)  );

    }

    public static int findSubscriberGroups(List<String> arrayList) {
        if (null == arrayList || arrayList.isEmpty()) return 0;
        int count = 0;
    ...
    private static void alignedGroups(int[][] isConnected, boolean[] isReached, int v) {
        isReached[v] = true;
        for (int i = 0; i < isConnected.length; i++) {
            if (isConnected[v][i] == 1 && !isReached[i]) alignedGroups(isConnected, isReached, i);
        }
    }
}
